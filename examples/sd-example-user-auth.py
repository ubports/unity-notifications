...
##
## Run:
##    chmod +x sd-example-user-auth.py
...
def bus_acquired(bus, name):
	...
	login_action = Gio.SimpleAction.new_stateful("login", GLib.VariantType.new("s"), GLib.Variant.new_string(""));
	login_action.connect ("change-state", login_changed);
	actions.insert (login_action);
...
def pushNotification (title, body, icon):
	...
	n.set_hint ("x-canonical-private-affirmative-tint", GLib.Variant.new_string("true"));
	n.set_hint ("x-canonical-private-rejection-tint", GLib.Variant.new_string("true"));
...
if __name__ == '__main__':
	if not Notify.init("sd-example-user-auth"):
		sys.exit (1)
